...
class RNN2:
    ...
    def __init__(self, classDim=0, wordDim=0, hiddenDim=0, senLen=0, batchSize=0, truncate=-1):
        ...
        W = np.random.uniform(-np.sqrt(1./hiddenDim), np.sqrt(1./hiddenDim), (8, hiddenDim, hiddenDim)) #W[0-1].dot(x), W[2-3].(i,f,o,c)
        B = np.random.uniform(-np.sqrt(1./hiddenDim), np.sqrt(1./hiddenDim), (2, hiddenDim))             #B[0-1] for W[0-1]
        V = np.random.uniform(-np.sqrt(1./hiddenDim), np.sqrt(1./hiddenDim), (hiddenDim, classDim))         #LR W and b
    ...
    def __theano_build__(self):
        ...
        x1_mask = T.dmatrix('x1_mask')    #mask
        x2_mask = T.dmatrix('x2_mask')
        y   = T.ivector('y')     #label
    
        #Embdding words
        _E1 = E.dot(W[0]) + B[0]
        _E2 = E.dot(W[1]) + B[1]
        statex1 = _E1[x1.flatten(), :].reshape([x1.shape[0], x1.shape[1], hiddenDim])
        statex2 = _E2[x2.flatten(), :].reshape([x2.shape[0], x2.shape[1], hiddenDim])

        def RNNLayer2(x, mx, ps1, ps2, Ws12, Ws21, Ws22):
            s = ps1.dot(Ws12) + x
            s = T.tanh(s)
            s = mx[:, None] * s + (1.-mx[:,None]) * ps1
        
            s = ps2.dot(Ws22) + s.dot(Ws21)
            s = T.tanh(s)
            s = mx[:, None] * s + (1.-mx[:,None]) * ps2
        
            return s  # size = sample * hidden : 3 * 4
        ...
        h2, updates = theano.scan(
            ...
            outputs_info=[dict(initial=h1[-1]),
                          dict(initial=T.zeros([self.batchSize, self.hiddenDim]))],  #using h[end] to initialize the second LSTM
            non_sequences=[W[5], W[6], W[7]])
       
        #calculate Attention based output
        _s = T.nnet.softmax(h2[-1].dot(V) + b)
        _p = T.argmax(_s, axis=1)
        _c = T.nnet.categorical_crossentropy(_s, self.C[y,:])
    ...
    def save_model(self, outfile):
        np.savez(outfile,
            ...
            V = self.V.get_value(),
            b = self.b.get_value())
        print "Saved RNN2' parameters to %s." % outfile

    def load_model(self, path):
        npzfile = np.load(path)
        E, W, B, V, b = npzfile["E"], npzfile["W"], npzfile["B"], npzfile["V"], npzfile["b"]
        self.hiddenDim, self.wordDim, self.classDim = E.shape[1], E.shape[0], V.shape[1]
