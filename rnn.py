...
class RNN:
    ...
    def __init__(self, classDim=0, wordDim=0, hiddenDim=0, senLen=0, batchSize=0, truncate=-1):
        ...
        W = np.random.uniform(-np.sqrt(1./hiddenDim), np.sqrt(1./hiddenDim), (4, hiddenDim, hiddenDim)) #W[0-1].dot(x), W[2-3].(i,f,o,c)
    ...
    def __theano_build__(self):
        ...
        def RNNLayer(x, mx, ps, Ws):
            s = ps.dot(Ws) + x
            s = T.tanh(s)
            s = mx[:, None] * s + (1.-mx[:,None]) * ps
    ...
    def save_model(self, outfile):
        np.savez(outfile,
            ...
            V = self.V.get_value(),
            b = self.b.get_value())
        print "Saved RNN' parameters to %s." % outfile
